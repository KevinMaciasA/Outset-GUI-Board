...
def do_gui_media_update(force_mode):
    sys.excepthook = handle_exception
    isForced = force_mode;
    ensure_temp_dir()
    status = ensure_adb_connected()
    if(error):
        return status
    version, url = get_media_required()
    if(error):
        return status
    installed, status = check_media_already_installed(version)
    if(installed or error):
        return status
    status = download_media(version, url)
...
def get_media_required():
    ...
    if(error):
        status = "Unable to find installed GUI on console - install it first!"
        return(0, "")
...
def download_media(version, url):
    ...
    if ret is not None:
        error = True
        return('Media download corrupt, first damaged file: %s' % ret)
...
def check_media_already_installed(version):
    if isForced:
        print('Skipping media check, force option specified')
        return (False, "OK")
    ...
    for one_line in contents.split('\n'):
        if one_line.find(MEDIA_MANIFEST_NAME) > -1 and one_line.lower().find("no such file") == -1:
            ...
            return (True, 'GUI board already has correct media installed, bailing out')
...
def adb(command, wait=True, accept_error_one=False, noisy=True):
    ...
    if wait:
        ...
        if exit_code > 0 and not (exit_code == 1 and accept_error_one):
            die('unexpected exit code %d from command: %s' % (exit_code, command))
